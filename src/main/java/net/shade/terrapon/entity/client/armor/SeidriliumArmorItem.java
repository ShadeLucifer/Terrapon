...
import net.shade.terrapon.entity.client.armor.SeidriliumArmorRenderer;
...
public class SeidriliumArmorItem extends ArmorItem implements GeoItem{
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);
    private static final Map<ArmorMaterial, MobEffectInstance> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, MobEffectInstance>())
                    .put(TerraponArmorMaterials.SEIDRILIUM, new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 2, 2))
                    .build();

    public SeidriliumArmorItem(ArmorMaterial material, Type type, Properties properties) {
    ...
    public void initializeClient(Consumer<IClientItemExtensions> consumer) {
        consumer.accept(new IClientItemExtensions() {
            private SeidriliumArmorRenderer renderer;

            @Override
            public @NotNull HumanoidModel<?> getHumanoidArmorModel(LivingEntity livingEntity, ItemStack itemStack,
                                                                   EquipmentSlot equipmentSlot, HumanoidModel<?> original) {
                if (this.renderer == null)
                    this.renderer = new SeidriliumArmorRenderer();
